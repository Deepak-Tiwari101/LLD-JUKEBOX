...
public class PlaylistService {
    ...
    private List<Song> getListOfSongsfromIdList(List<Long> songIds) {
        ...
        for (Long id : songIds) list.add(songRepo.findById(id).orElseThrow(() -> new RuntimeException("Song with id: " + id + " not found!")));
        return list;
    }

    public void addSongToPlaylist(String playlistName, Long songId) {
        Song song = songRepo.findById(songId).orElseThrow(() -> new RuntimeException("Song with id: " + songId + " not found!"));
        Playlist playlist = playlistRepo.findByName(playlistName).orElseThrow(() -> new RuntimeException("Playlist with name: " + playlistName + " not found!"));
    ...
    public void loadPlaylist(String playlistName) {
        ...
        if (playlistRepo.getLoadedPlaylist() != null) playlist.setState(PlaylistStatus.NOT_LOADED);
    ...
    public void nextSong() {
        Playlist playlist = playlistRepo.getLoadedPlaylist();
        List<Song> songList = playlist.getSongsList();
        songList.get(songNavigationIndex % songList.size()).setSongStatus(SongStatus.STOPPED); // stop the current playing
        songNavigationIndex++;
    ...
    public void prevSong() {
        ...
        songList.get(songNavigationIndex % songList.size()).setSongStatus(SongStatus.STOPPED); // stop the current playing
        songNavigationIndex--;
        if(songNavigationIndex < 0) songNavigationIndex = songList.size() - 1; // to handle negative index. as soon as the index drops below 0, assign it as the last index of the list
        songList.get(songNavigationIndex % songList.size()).setSongStatus(SongStatus.PLAYING); // start the previous song
        System.out.println(songList.get(songNavigationIndex % songList.size()) + " is playing!");
    }
